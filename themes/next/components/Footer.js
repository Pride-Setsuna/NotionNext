...
const Footer = ({ title }) => {
  ...
  return (
    <footer className='relative z-10 dark-bg-gray-800 flex-shrink-0 justify-center text-center m-auto w-full leading-6 text-sm p-6 bg-white dark-text-gray-400'>
      <span>
        <DarkModeButton />
        <i className='fas fa-copyright' /> {`${currentYear}`} <span className='mx-1 animate-pulse'><i className='fas fa-heart' /></span> <a href={BLOG.LINK} className='underline font-bold '>{BLOG.AUTHOR}</a>.<br />
        {BLOG.BEI_AN && <><i className='fas fa-shield-alt' /> <a href='https://beian.miit.gov.cn/' className='mr-2'>{BLOG.BEI_AN}</a><br /></>}
        {/* 删除下面的代码，不再显示软件驱动和版本号信息
